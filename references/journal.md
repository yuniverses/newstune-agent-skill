...
- **Read or visibility endpoints return 404**: on the current API, first verify the series/episode IDs and ownership. `bind`/`collect`/`publish` retain a compatibility degradation to `podcast.json`/`ledger.json` for known older deployments, but `{ "reason": "ENDPOINT_NOT_DEPLOYED" }` is not proof that a current resource exists.
